...
// Template rendering functions (duplicated for standalone build)
...
function renderStep(step) {
  ...
  if (step.fields || step.likert) {
    ...
    if (step.likert) {
      content += renderLikertTable(step.likert);
    }
...
// Generate full HTML
const stepsHtml = SURVEY_STEPS.map(renderStep).join('\n');

const navItems = SURVEY_STEPS.map(step => `
...
// Output path
const outputPath = join(__dirname, '..', 'views', 'survey-generated.html');

// Write header comment
const header = `<!--
  AUTO-GENERATED FILE - DO NOT EDIT DIRECTLY
  Edit src/data/survey-data.js instead, then run: node scripts/build-survey-html.js
-->
`;

writeFileSync(outputPath, header + stepsHtml);
console.log(`Generated: ${outputPath}`);
console.log(`Steps: ${SURVEY_STEPS.length}`);
